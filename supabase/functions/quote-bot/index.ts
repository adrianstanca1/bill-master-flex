...
serve(async (req) => {
  ...
  try {
    ...
    const aiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "gpt-4o-mini", temperature: 0.3, messages: [ { role: "system", content: "You are QuoteBot, a UK construction estimator." }, { role: "user", content: prompt } ] })
    });
